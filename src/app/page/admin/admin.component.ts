...
export class AdminComponent {
      ...
      .mutate({
        ...
        variables: {
          ...
          updateProductInput: {
            ...
            image:
              'https://www.ucn.cl/wp-content/uploads/2018/05/Escudo-UCN-Full-Color.png',
  ...
  loadProductAdmin(): void {
    this.querySubscriptionAdmin = this.apolloProduct
      .watchQuery<any>({
  ...
  getProductDetailsById(productId: string): any {
    ...
    const selectedProductAdmin = this.AllProductsAdmin.find(
      (product) => product.id === parseInt(productId)
    );
    // Ahora puedes acceder a los detalles del producto seleccionado
    if (selectedProductAdmin) {
      const { name, category, price, image, username, description } =
